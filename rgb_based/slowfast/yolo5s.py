...
# 検出関連の設定
PERSON_THRESHOLD = 0.5  # 人物検出の閾値
ACTION_THRESHOLD = 0.3  # 行動検出の閾値
...
def realtime_action_detection(model, device=None):
    ...
    try:
        ...
        yolo_detector.conf = PERSON_THRESHOLD  # 検出信頼度閾値
        yolo_detector.classes = [0]  # 0:人物クラスのみ
        yolo_detector.eval()
        print("YOLOv5人物検出器を読み込みました")
    except Exception as e:
        print(f"YOLOv5の読み込みエラー: {e}")
        print("システムを終了します")
        return
    ...
    cv2.namedWindow('SlowFast + YOLOv5 Action Detection', cv2.WINDOW_NORMAL)
    cv2.resizeWindow('SlowFast + YOLOv5 Action Detection', window_width, window_height)
    
    print("SlowFast + YOLOv5による行動検出を開始します。'q'キーで終了します。")
    ...
    while True:
        ...
        # バッファが長すぎる場合は古いフレームを削除
        if len(frame_buffer) > NUM_FRAMES:
            frame_buffer.pop(0)
        
        # 検出間隔に達した場合、またはバッファが満杯になった場合に検出実行
        should_detect = (
            len(frame_buffer) == NUM_FRAMES and 
            (frame_count - last_detection_frame) >= DETECTION_INTERVAL
        )
        ...
        if should_detect:
            ...
            # YOLOv5による人物検出
            current_frame = cv2.resize(frame, (640, 480))  # YOLOv5用のサイズ
            yolo_results = yolo_detector(current_frame)
            pred_boxes = []
            pred_scores = []
            ...
            if len(yolo_results.xyxy[0]) > 0:
                ...
                for det in yolo_results.xyxy[0]:
                    if det[5] == 0:  # クラス0は人物
                        x1, y1, x2, y2, score, _ = det.cpu().numpy()
        ...
        # システム情報の表示
        cv2.putText(display_frame, "SlowFast + YOLOv5 Action Detection", (10, 30),
        ...
        cv2.rectangle(display_frame, (window_width-150, 10), (window_width-10, 120), status_color, 3)
        ...
        # フレーム表示
        cv2.imshow('SlowFast + YOLOv5 Action Detection', display_frame)
...
def main():
    """メイン処理"""
    # パラメータの設定
    parser = argparse.ArgumentParser(description="SlowFast + YOLOv5 Action Detection System")
    parser.add_argument("--output_dir", type=str, default="./output_slowfast_yolo_detection",
                        help="出力ディレクトリ")
    ...
    try:
        ...
        # リアルタイム検出の実行
        print("YOLOv5による人物検出とSlowFastによる行動認識を開始します...")
        ...
        print("3. インターネット接続を確認（初回実行時はモデルをダウンロードします）")
        print("4. YOLOv5の依存関係がインストールされているか確認")

if __name__ == "__main__":
    print("=" * 60)
    print("SlowFast + YOLOv5 リアルタイム行動検出システム")
    print("=" * 60)
    print("このシステムは以下の技術を組み合わせます:")
    print("- YOLOv5: 高精度な人物検出")
